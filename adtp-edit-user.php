...
function wpuf_edit_users() {
    ...
    if ( is_user_logged_in() ) {
        ...
        if ( current_user_can( 'edit_users' ) ) {
            ...
            switch ($action) {
                case 'edit':
                    ...
                    if ( $user_id && $userdata ) {
                        ...
                        printf( __( "User doesn't exists", 'adtp' ) );
                    }
                    break;
            ...
            printf( __( "You don't have permission for this purpose", 'adtp' ) );
        }
...
function wpuf_show_users() {
    ...
    <?php if ( $users ) : ?>
        <table class="wpuf-table" cellpadding="0" cellspacing="0">
            <tr>
                <th><?php _e( 'Username', 'adtp' ); ?></th>
                <th><?php _e( 'Action', 'adtp' ); ?></th>
            </tr>
            <?php foreach ($users as $user): ?>
                <tr>
                    <td><a href="<?php echo get_author_posts_url( $user->ID ); ?>"><?php printf( esc_attr__( '%s', 'adtp' ), $user->display_name ); ?></td>
                    <td>
                        <a href="<?php echo wp_nonce_url(get_permalink() . '?action=edit&user_id='. $user->ID, 'wpuf_edit_user'); ?>"><?php _e( 'Edit', 'adtp' ); ?></a>
                        <a href="<?php echo wp_nonce_url( the_permalink( 'echo=false' ) . "?action=del&user_id=" . $user->ID, 'wpuf_del_user' ) ?>" onclick="return confirm('Are you sure to delete this user?');"><span style="color: red;"><?php _e( 'Delete', 'adtp' ); ?></span></a>
                    </td>
                </tr>
...
function wpuf_add_user() {
    ...
    <?php if ( current_user_can( 'create_users' ) ) : ?>
        ...
        <h3><?php _e( 'Add New User', 'adtp' ); ?></h3>
            ...
            <ul class="wpuf-post-form">
                ...
                <li>
                    <label for="user_email">
                        <?php _e( 'Email', 'adtp' ); ?> <span class="required">*</span>
                    </label>
                ...
                <li>
                    <label>&nbsp;</label>
                    <input class="wpuf_submit" type="submit" name="wpuf_new_user_submit" value="<?php echo esc_attr( __( 'Add New User', 'adtp' ) ); ?>">
                </li>
